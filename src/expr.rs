...
impl Printer<'_> {
    pub fn expr_without_fixup(&mut self, expr: &Expr) {
    ...
    fn prefix_subexpr_await(
        ...
        if !(beginning_of_line && is_short_ident(&expr.base, self.indent())) {
        ...
        if let Some((_else_token, else_branch)) = &expr.else_branch {
            ...
            loop {
                ...
                match else_branch {
                    ...
                    other => self.expr_as_small_block(other, self.indent()),
                }
                break;
            }
    ...
    fn prefix_subexpr_method_call(
        ...
        if !(beginning_of_line && is_short_ident(&expr.receiver, self.indent())) {
            self.scan_break(BreakToken {
                no_break: self.ends_with('.').then_some(' '),
                ..BreakToken::default()
            });
